...
enum {
    ...
    Qidle = 0, Qinstall, Qrun, Qdone, Qclose, Qfree,
    ...
    Itdlenmask  = 0xFFF, Itdioc = 0x00008000, /* interrupt on complete */
    ...
    Itdout       = 0, Itdmaxpktshift = 0, /* max packet (buffer[1]) */
    ...
    Stdlenmask   = 0x3FF, Stdactive = 0x00000080, /* active */
    ...
    Tdlenmask  = 0x7FFF, Tdmaxpkt = 0x5000, /* max buffer for a Td */
    Tdioc      = 0x00008000,    /* interrupt on complete */
    Tdpgshift  = 12,            /* current page */
    Tdpgmask   = 7,      Tderr1 = 0x00000400, /* bit 0 of error counter */
    ...
    Qhmplshift  = 16, Qhhrl = 0x00008000, /* head of reclamation list */
    ...
    Qhmultmask  = 3, Qhportshift = 23, /* hub port number */
...
static void tdlinktd(Td *td, Td *next)
{
    td->next = next;
    td->alink = Lterm;
    if (next == nil)
...
static void qhsetaddr(Qh *qh, ulong addr)
{
    ulong eps0;
...
static int pickschedq(Qtree *qt, int pollival, ulong bw, ulong limit)
{
    int i, j, d, upperb, q;
...
static int schedq(Ctlr *ctlr, Qh *qh, int pollival)
{
    int q;
    Qh *tqh;
    ulong bw;
...
static void unschedq(Ctlr *ctlr, Qh *qh)
{
    int q;
    Qh * prev, *this, *next;
    Qh **l;
...
static ulong qhmaxpkt(Qh *qh)
{
    return (qh->eps0 >> Qhmplshift) & Qhmplmask;
}

static void qhsetmaxpkt(Qh *qh, int maxpkt)
{
    ulong eps0;
 ...
 */
static void qhcoherency(Ctlr *ctlr)
{
    int i;
...
static void qhfree(Ctlr *ctlr, Qh *qh)
{
    Td *td, *ltd;
    Qh *q;
...
static void qhlinktd(Qh *qh, Td *td)
{
    ulong csw;
    int i;
...
seprintitd(char *s, char *se, Itd *td)
{
    ...
    s = seprint(s, se, " %s ep %uld dev %uld max %uld mult %uld", rw,
                (b0 >> 8) & Epmax, (b0 & Devmax), td->buffer[1] & 0x7ff, b1 & 3);
...
static long maxtdlen(Td *td)
{
    return (td->csw >> Tdlenshift) & Tdlenmask;
}

static long tdlen(Td *td)
{
    if (td->data == nil)
        return 0;
...
static void dumptd(Td *td, char *pref)
{
    char buf[256];
    char *se;
    int i;
...
static void isodump(Isoio* iso, int all)
{
    Itd *td, *tdi, *tdu;
    Sitd *std, *stdi, *stdu;
    char buf[256];
...
static void dump(Hci *hp)
{
    ...
    opio = ctlr->opio;
    ilock(ctlr);
    print("ehci port %#p frames %#p (%d fr.) nintr %d ntdintr %d", ctlr->capio,
          ctlr->frames, ctlr->nframes, ctlr->nintr, ctlr->ntdintr);
    print(" nqhintr %d nisointr %d\n", ctlr->nqhintr, ctlr->nisointr);
    print("\tcmd %#lux sts %#lux intr %#lux frno %uld", opio->cmd, opio->sts,
          opio->intr, opio->frno);
...
static void itdinit(Isoio *iso, Itd *td)
{
    int p, t;
    ulong pa, tsize, size;
...
static void sitdinit(Isoio *iso, Sitd *td)
{
...
static int isohsinterrupt(Ctlr *ctlr, Isoio *iso)
{
    int err, i, nframes, t;
    Itd *tdi;
...
static int isofsinterrupt(Ctlr *ctlr, Isoio *iso)
{
    int err, i, nframes;
    Sitd *stdi;
    ...
    coherence();
    if (isocanwrite(iso) || isocanread(iso)) {
        diprint("wakeup iso %#p tdi %#p tdu %#p\n", iso, iso->stdi, iso->stdu);
        wakeup(iso);
    }
    return 1;
}

static int qhinterrupt(Ctlr *ctlr, Qh *qh)
{
    Td *td;
    int err;
...
static int ehciintr(Hci *hp)
{
    ...
    if ((sts & (Serrintr | Sintr)) != 0) {
        ctlr->ntdintr++;
        if (ehcidebug > 1) {
            print("ehci port %#p frames %#p nintr %d ntdintr %d", ctlr->capio,
...
static void interrupt(Ureg*, void* a)
{
    ehciintr(a);
}

static int portenable(Hci *hp, int port, int on)
{
    Ctlr *ctlr;
    Eopio *opio;
    int s;
 ...
 */
static void portlend(Ctlr *ctlr, int port, char *ss)
{
    Eopio *opio;
    ulong s;
...
static int portreset(Hci *hp, int port, int on)
{
    ulong *portscp;
    Eopio *opio;
    Ctlr *ctlr;
...
static int portstatus(Hci *hp, int port)
{
    int s, r;
    Eopio *opio;
    Ctlr *ctlr;
...
static void xdump(char* pref, void *qh)
{
    int i;
    ulong *u;
...
static long episohscpy(Ctlr *ctlr, Ep *ep, Isoio* iso, uchar *b, long count)
{
    int nr;
    long tot;
    Itd *tdu;
...
static long episofscpy(Ctlr *ctlr, Ep *ep, Isoio* iso, uchar *b, long count)
{
    int nr;
    long tot;
    Sitd *stdu;
...
static long episoread(Ep *ep, Isoio *iso, void *a, long count)
{
    Ctlr *ctlr;
    uchar *b;
    long tot;
 ...
 */
static long putsamples(Isoio *iso, uchar *b, long count)
{
    long tot, n;
 ...
 */
static long episowrite(Ep *ep, Isoio *iso, void *a, long count)
{
    Ctlr *ctlr;
    uchar *b;
    int tot, nw;
...
static int nexttoggle(int toggle, int count, int maxpkt)
{
    int np;
...
static void epiowait(Hci *hp, Qio *io, int tmout, ulong load)
{
    Qh *qh;
    int timedout;
    Ctlr *ctlr;
 ...
 */
static long epio(Ep *ep, Qio *io, void *a, long count, int mustlock)
{
    ...
    ddeprint("epio: %s ep%d.%d io %#p count %ld load %uld\n",
             io->tok == Tdtokin ? "in" : "out", ep->dev->nb, ep->nb, io, count,
             ctlr->load);
...
static long epread(Ep *ep, void *a, long count)
{
    Ctlio *cio;
    Qio *io;
    Isoio *iso;
 ...
 */
static long epctlio(Ep *ep, Ctlio *cio, void *a, long count)
{
    uchar *c;
    long len;
...
static long epwrite(Ep *ep, void *a, long count)
{
    Qio *io;
    Ctlio *cio;
    Isoio *iso;
...
static void isofsinit(Ep *ep, Isoio *iso)
{
    long left;
    Sitd *td, *ltd;
    int i;
...
static void isohsinit(Ep *ep, Isoio *iso)
{
    int ival, p;
    long left;
    ulong frno, i, pa;
    ...
    for (i = 0; i < iso->nframes; i++) {
        ...
        td->buffer[0] = PADDR(iso->data) & ~0xFFF | ep->nb << Itdepshift
...
static void isoopen(Ctlr *ctlr, Ep *ep)
{
...
static void cancelio(Ctlr *ctlr, Qio *io)
{
    Qh *qh;
...
static void cancelisoio(Ctlr *ctlr, Isoio *iso, int pollival, ulong load)
{
 ...
 */
static void mkqhtree(Ctlr *ctlr)
{
