...
struct usb_kbd_pdata {
	...
	uint32	usb_in_pointer;
	uint32	usb_out_pointer;
	uint8		usb_kbd_buffer[USB_KBD_BUFFER_LEN];
...
static uint32 usb_kbd_service_key(struct usb_device *dev, int i, int up)
{
	uint32 res = 0;
	struct usb_kbd_pdata *data = dev->privptr;
	uint8 *new;
...
static int usb_kbd_probe(struct usb_device *dev, unsigned int ifnum)
{
	...
	USB_KBD_cprintf("USB KBD: found set protocol...\n");

	data = malloc(sizeof(struct usb_kbd_pdata));
	if (!data) {
		cprintf("USB KBD: Error allocating private data\n");
		return 0;
	}
	...
	USB_KBD_cprintf("USB KBD: found set idle...\n");
...
int drv_usb_kbd_init(void)
{
	...
	for (i = 0; i < USB_MAX_DEVICE; i++) {
		...
		USB_KBD_cprintf("USB KBD: found set up device.\n");
		old_dev = stdio_get_by_name(DEVNAME);
		if (old_dev) {
			/* Already registered, just return ok. */
			USB_KBD_cprintf("USB KBD: is already registered.\n");
			return 1;
		}

		/* Register the keyboard */
		USB_KBD_cprintf("USB KBD: register.\n");
