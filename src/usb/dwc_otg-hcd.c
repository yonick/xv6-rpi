...
static uint8 *allocated_buffer;
static uint8 *aligned_buffer;
static uint8 *status_buffer;
...
{
	cprintf("DWC OTG: hang at line %d: %08x\n", line, d);
	while(1)
	{
		micro_delay(10);
	}
...
int usb_lowlevel_init(int index, void **controller)
{
	 ...
	 */
	allocated_buffer = (uint8*)malloc(DWC_OTG_HCD_STATUS_BUF_SIZE + DWC_OTG_HCD_DATA_BUF_SIZE + 8);
	uint32 addr = (uint32)allocated_buffer;
	aligned_buffer = (uint8*) ((addr + 7) & ~7);
	status_buffer = (uint8*)((uint32)aligned_buffer + DWC_OTG_HCD_DATA_BUF_SIZE);
	int i, j;
	hprt0_data_t hprt0 = {.d32 = 0 };

	root_hub_devnum = 0;
	memset(&g_core_if, 0, sizeof(g_core_if));
	dwc_otg_cil_init(&g_core_if, (uint32*)0x20980000);

	if ((g_core_if.snpsid & 0xFFFFF000) !=
		0x4F542000) {
		cprintf("SNPSID is invalid (not DWC OTG device): %08x\n", g_core_if.snpsid);
		return -1;
	}
	...
	case RH_GET_STATUS | RH_OTHER | RH_CLASS:
		{
			...
			*(uint32 *)buffer = cpu_to_le32(port_status |
						(port_change << 16));
			len = 4;
		}
		break;
		    ...
		    int len)
{
	...
	uint32 max_hc_xfer_size = g_core_if.core_params->max_transfer_size;
	uint16 max_hc_pkt_count = g_core_if.core_params->max_packet_count;
	uint32 xfer_len;
	uint32 num_packets;
	int stop_transfer = 0;

	if (len > DWC_OTG_HCD_DATA_BUF_SIZE) {
		cprintf("submit_bulk_msg: %d is more then available buffer size(%d)\n", len, DWC_OTG_HCD_DATA_BUF_SIZE);
		dev->status = 0;
		dev->act_len = done;
		return -1;
	}
	...
	while ((done < len) && !stop_transfer) {
		...
		dwc_write_reg32(&hc_regs->hcdma, (uint32)aligned_buffer);
		       ...
		       int len,struct devrequest *setup)
{
	...
	if (len > DWC_OTG_HCD_DATA_BUF_SIZE) {
		cprintf("submit_control_msg: %d is more then available buffer size(%d)\n", len, DWC_OTG_HCD_DATA_BUF_SIZE);
		dev->status = 0;
		dev->act_len = done;
		return -1;
	}
	...
	dwc_write_reg32(&hc_regs->hcdma, (uint32)status_buffer);
...
{
	cprintf("dev = %p pipe = %#lx buf = %p size = %d int = %d\n", dev, pipe,
	       buffer, len, interval);
	return -1;
}
