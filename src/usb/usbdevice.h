...
#if 1
#define usberr(fmt,args...) serial_cprintf("ERROR: %s(), %d: "fmt"\n",__FUNCTION__,__LINE__,##args)
#else
#define usberr(fmt,args...) do{}while(0)
#endif

#if 0
#define usbdbg(fmt,args...) serial_cprintf("debug: %s(), %d: "fmt"\n",__FUNCTION__,__LINE__,##args)
#else
#define usbdbg(fmt,args...) do{}while(0)
#endif

#if 0
#define usbinfo(fmt,args...) serial_cprintf("info: %s(), %d: "fmt"\n",__FUNCTION__,__LINE__,##args)
#else
...
#ifndef inb
#define inb(p)	     (*(volatile uint8*)(p))
#endif

#ifndef outb
#define outb(val,p)  (*(volatile uint8*)(p) = (val))
#endif

#ifndef inw
#define inw(p)	     (*(volatile uint16*)(p))
#endif

#ifndef outw
#define outw(val,p)  (*(volatile uint16*)(p) = (val))
#endif

#ifndef inl
#define inl(p)	     (*(volatile uint32*)(p))
#endif

#ifndef outl
#define outl(val,p)  (*(volatile uint32*)(p) = (val))
#endif
...
{
	serial_cprintf("urb %p\n", (u));
	serial_cprintf("\tendpoint %p\n", u->endpoint);
	serial_cprintf("\tdevice %p\n", u->device);
	serial_cprintf("\tbuffer %p\n", u->buffer);
	serial_cprintf("\tbuffer_length %d\n", u->buffer_length);
	serial_cprintf("\tactual_length %d\n", u->actual_length);
	serial_cprintf("\tstatus %d\n", u->status);
	serial_cprintf("\tdata %d\n", u->data);
}

static inline void print_usb_device_request(struct usb_device_request *r)
{
	serial_cprintf("usb request\n");
	serial_cprintf("\tbmRequestType 0x%2.2x\n", r->bmRequestType);
	if ((r->bmRequestType & USB_REQ_DIRECTION_MASK) == 0)
		serial_cprintf("\t\tDirection : To device\n");
	else
		serial_cprintf("\t\tDirection : To host\n");
	if ((r->bmRequestType & USB_TYPE_STANDARD) == USB_TYPE_STANDARD)
		serial_cprintf("\t\tType      : Standard\n");
	...
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
		...
		serial_cprintf("\t\tRecipient : Device\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_INTERFACE)
		serial_cprintf("\t\tRecipient : Interface\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_ENDPOINT)
		serial_cprintf("\t\tRecipient : Endpoint\n");
	if ((r->bmRequestType & USB_REQ_RECIPIENT_MASK) ==
	    USB_REQ_RECIPIENT_OTHER)
		serial_cprintf("\t\tRecipient : Other\n");
	serial_cprintf("\tbRequest      0x%2.2x\n", r->bRequest);
	if (r->bRequest == USB_REQ_GET_STATUS)
		serial_cprintf("\t\tGET_STATUS\n");
	else if (r->bRequest == USB_REQ_SET_ADDRESS)
		serial_cprintf("\t\tSET_ADDRESS\n");
	else if (r->bRequest == USB_REQ_SET_FEATURE)
		serial_cprintf("\t\tSET_FEATURE\n");
	else if (r->bRequest == USB_REQ_GET_DESCRIPTOR)
		serial_cprintf("\t\tGET_DESCRIPTOR\n");
	else if (r->bRequest == USB_REQ_SET_CONFIGURATION)
		serial_cprintf("\t\tSET_CONFIGURATION\n");
	else if (r->bRequest == USB_REQ_SET_INTERFACE)
		serial_cprintf("\t\tUSB_REQ_SET_INTERFACE\n");
	else
		serial_cprintf("\tUNKNOWN %d\n", r->bRequest);
	...
	serial_cprintf("\twLength       0x%4.4x\n", r->wLength);
}
#else
/* stubs */
#define print_urb(u)
