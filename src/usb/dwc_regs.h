...
#define MAX_CHANS	 		16	/*Maximum number of HostChannels */
...
struct hc_regs {
	...
	uint32 hcintmsk;/*Interrupt Mask Register.*/
	uint32 hctsiz;/*Transfer Size Register.*/
	uint32 hcdma;/*DMA Address Register.*/
	uint32 reserved;
	uint32 hcdmab;/*DMA Buffer Address Register.*/
};

struct dwc_regs {
	/** DWC_otg Core registers (0x000-0x140).
	 * The dwc_otg_core_global_regs structure defines the size
	 * and relative field offsets for the Core Global registers.
	 */
	uint32 gotgctl; /*OTG Control and Status Register. */
	uint32 gotgint; /*OTG Interrupt Register */
	uint32 gahbcfg; /*Core AHB Configuration Register. */
	uint32 gusbcfg; /*Core USB Configuration Register. */
	uint32 grstctl; /*Core Reset Register.*/
	uint32 gintsts; /*Core Interrupt Register.*/
	uint32 gintmsk; /*Core Interrupt Mask Register.*/
	uint32 grxstsr; /*Receive Status Queue Read Register (Read Only)*/
	uint32 grxstsp; /*Receive Status Queue Read & POP Register (Read Only).*/
	uint32 grxfsiz; /*Receive FIFO Size Register.*/
	uint32 gnptxfsiz; /*Non Periodic Transmit FIFO Size Register.*/
	uint32 gnptxsts; /*Non Periodic Transmit FIFO/Queue Status Register (Read Only)*/
	uint32 gi2cctl; /*I2C Access Register*/
	uint32 gpvndctl; /*PHY Vendor Control Register */
	uint32 ggpio; /*General Purpose Input/Output Register. */
	uint32 guid; /*User ID Register.*/
	uint32 gsnpsid; /*Synopsys ID Register (Read Only).*/
	uint32 ghwcfg1; /*User HW Config1 Register (Read Only).*/
	uint32 ghwcfg2; /*User HW Config2 Register (Read Only).*/
	uint32 ghwcfg3; /*User HW Config3 Register (Read Only).*/
	uint32 ghwcfg4; /*User HW Config4 Register (Read Only).*/
	uint32 glpmcfg; /* Core LPM Configuration register */
	uint32 reserved[42]; /* Reserved  <i>Offset: 058h-0FFh</i> */
	uint32 hptxfsiz; /* Host Periodic Transmit FIFO Size Register.  */
	uint32 dptxfsiz_dieptxf[15];/* Device Periodic Transmit FIFO#n Register if dedicated fifos
	 are disabled, otherwise Device Transmit FIFO#n Register. */
	 ...
	 */
	uint32 hcfg;/* Host Configuration Register.*/
	uint32 hfir;/* Host Frame Interval Register.*/
	uint32 hfnum;/* Host Frame Number / Frame Remaining Register.*/
	uint32 reserved40C;/* Reserved.*/
	uint32 hptxsts;/* Host Periodic Transmit FIFO/ Queue Status Register*/
	uint32 haint;/* Host All Channels Interrupt Register*/
	uint32 haintmsk;/* Host All Channels Interrupt Mask Register*/
	uint32 hflbaddr;/* Host Frame List Base Address Register */

	char pad1[0x440 - 0x420];
	uint32 hport0; /*host prot 0 control and status*/
...
#define CHANINT(n)	(1<<(n))
	...
	Epbulk = 2 << 18,
	Epintr = 3 << 18,
	Multicnt = 0x3 << 20, /* transactions per uframe */
	...
	Stall = 1 << 3, /* received stall from endpoint*/
	Nak = 1 << 4,	/*received Nak from ep*/
	Ack = 1 << 5,	/*received ACK from ep*/
	Nyet = 1 << 6,	/*received Nyet(not ready) from ep*/
	Xacterr = 1 << 7, /* transaction error (crc, t/o, bit stuff, eop) */
	Bblerr = 1 << 8,	/*ballle error*/
	Frmovrun = 1 << 9,	/*frame overrun*/
	Datatglerr = 1 << 10,
	Bna = 1 << 11,
	Xcs_xact = 1 << 12,
	Frm_list_roll = 1 << 13,
