 ...
 */
static inline uint32 dwc_otg_read_host_all_channels_intr(dwc_otg_core_if_t *
                                                           _core_if)
{
        return (dwc_read_reg32(&_core_if->host_if->host_global_regs->haint));
}

static inline uint32 dwc_otg_read_host_channel_intr(dwc_otg_core_if_t *
                                                      _core_if, int hc_num)
{
...
int dwc_otg_set_param_otg_cap(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 2)) {
		cprintf("warning: Wrong value for otg_cap parameter\n");
		cprintf("warning: otg_cap parameter must be 0,1 or 2\n");
		retval = -1;
		goto out;
	}
...
int dwc_otg_set_param_dma_desc_enable(dwc_otg_core_if_t * core_if, int32 val)
{
	int retval = 0;
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		cprintf("warning: Wrong value for dma_enable\n");
		cprintf("warning: dma_desc_enable must be 0 or 1\n");
		return -1;
	}
						   ...
						   int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: host_support_fs_low_power must be 0 or 1\n");
		return -1;
	}
...
int32 dwc_otg_get_param_host_support_fs_ls_low_power(dwc_otg_core_if_t *
						       core_if)
{
					  ...
					  int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: enable_dynamic_fifo must be 0 or 1\n");
		return -1;
	}
...
int dwc_otg_set_param_data_fifo_size(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 32, 32768)) {
		...
		cprintf("warning: data_fifo_size must be 32-32768\n");
		return -1;
	}
...
int dwc_otg_set_param_dev_rx_fifo_size(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 16, 32768)) {
		...
		cprintf("warning: dev_rx_fifo_size must be 16-32768\n");
		return -1;
	}
					      ...
					      int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 16, 32768)) {
		...
		cprintf("warning: dev_nperio_tx_fifo must be 16-32768\n");
		return -1;
	}
					...
					int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 16, 32768)) {
		...
		cprintf("warning: host_rx_fifo_size must be 16-32768\n");
		return -1;
	}
					       ...
					       int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 16, 32768)) {
		...
		cprintf("warning: host_nperio_tx_fifo_size must be 16-32768\n");
		return -1;
	}
					      ...
					      int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 16, 32768)) {
		...
		cprintf("warning: host_perio_tx_fifo_size must be 16-32768\n");
		return -1;
	}
					...
					int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 2047, 524288)) {
		...
		cprintf("warning: max_transfer_size must be 2047-524288\n");
		return -1;
	}
...
int dwc_otg_set_param_max_packet_count(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 15, 511)) {
		...
		cprintf("warning: max_packet_count must be 15-511\n");
		return -1;
	}
...
int dwc_otg_set_param_host_channels(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 1, 16)) {
		...
		cprintf("warning: host_channels must be 1-16\n");
		return -1;
	}
...
int dwc_otg_set_param_dev_endpoints(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 1, 15)) {
		...
		cprintf("warning: dev_endpoints must be 1-15\n");
		return -1;
	}
...
int dwc_otg_set_param_phy_type(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 2)) {
		...
		cprintf("warning: phy_type must be 0,1 or 2\n");
		return -1;
	}
...
int dwc_otg_set_param_speed(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: max_speed parameter must be 0 or 1\n");
		return -1;
	}
	if ((val == 0)
						...
						int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: host_ls_low_power_phy_clk must be 0 or 1\n");
		return -1;
	}
	...
	if ((val == DWC_HOST_LS_LOW_POWER_PHY_CLK_PARAM_48MHZ)
		...
		if(dwc_otg_param_initialized(core_if->core_params->host_ls_low_power_phy_clk)) {
			cprintf("error: %d invalid for host_ls_low_power_phy_clk. Check HW configuration.\n",
		     val);
		}
		val =
		    (dwc_otg_get_param_phy_type(core_if) ==
...
int dwc_otg_set_param_phy_ulpi_ddr(dwc_otg_core_if_t * core_if, int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: phy_upli_ddr must be 0 or 1\n");
		return -1;
	}
					...
					int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: phy_ulpi_ext_vbus must be 0 or 1\n");
		return -1;
	}
...
int dwc_otg_set_param_phy_utmi_width(dwc_otg_core_if_t * core_if, int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 8, 8) && DWC_OTG_PARAM_TEST(val, 16, 16)) {
		...
		cprintf("warning: phy_utmi_width must be 8 or 16\n");
		return -1;
	}
...
int dwc_otg_set_param_ulpi_fs_ls(dwc_otg_core_if_t * core_if, int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: ulpi_fs_ls must be 0 or 1\n");
		return -1;
	}
...
int dwc_otg_set_param_ts_dline(dwc_otg_core_if_t * core_if, int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: ts_dline must be 0 or 1\n");
		return -1;
	}
...
int dwc_otg_set_param_i2c_enable(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: i2c_enable must be 0 or 1\n");
		return -1;
	}
#ifndef NO_FS_PHY_HW_CHECK
	if (val == 1 && core_if->hwcfg3.b.i2c == 0) {
		if(dwc_otg_param_initialized(core_if->core_params->i2c_enable)) {
			cprintf("error: %d invalid for i2c_enable. Check HW configuration.\n",
		     val);
		}
		val = 0;
		retval = -1;
					     ...
					     int32 val, int fifo_num)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 4, 768)) {
		...
		cprintf("warning: dev_perio_tx_fifo_size must be 4-768\n");
		return -1;
	}
	...
	if (val > (dwc_read_reg32(&core_if->core_global_regs->dptxfsiz_dieptxf[fifo_num]))) {
		if(dwc_otg_param_initialized(core_if->core_params->dev_perio_tx_fifo_size[fifo_num])) {
			cprintf("error: `%d' invalid for parameter `dev_perio_fifo_size_%d'. Check HW configuration.\n",
		     val, fifo_num);
		}
					  ...
					  int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: en_multiple_tx_fifo must be 0 or 1\n");
		return -1;
	}

	if (val == 1 && core_if->hwcfg4.b.ded_fifo_en == 0) {
		if(dwc_otg_param_initialized(core_if->core_params->en_multiple_tx_fifo)) {
			cprintf("error: %d invalid for parameter en_multiple_tx_fifo. Check HW configuration.\n",
		     val);
		}
		val = 0;
		retval = -1;
				       ...
				       int fifo_num)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 4, 768)) {
		...
		cprintf("warning: dev_tx_fifo_size must be 4-768\n");
		return -1;
	}

	if (val > (dwc_read_reg32(&core_if->core_global_regs->dptxfsiz_dieptxf[fifo_num]))) {
		if(dwc_otg_param_initialized(core_if->core_params->dev_tx_fifo_size[fifo_num])) {
			cprintf("error: `%d' invalid for parameter `dev_tx_fifo_size_%d'. Check HW configuration.\n",
		     val, fifo_num);
		}
		val = (dwc_read_reg32(&core_if->core_global_regs->dptxfsiz_dieptxf[fifo_num]));
		retval = -1;
...
int dwc_otg_set_param_thr_ctl(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 7)) {
		...
		cprintf("warning: thr_ctl must be 0-7\n");
		return -1;
	}
	...
	if ((val != 0) &&
		...
		if(dwc_otg_param_initialized(core_if->core_params->thr_ctl)) {
			cprintf("error: %d invalid for parameter thr_ctl. Check HW configuration.\n",
		     val);
		}
		val = 0;
		retval = -1;
...
int dwc_otg_set_param_lpm_enable(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: lpm_enable must be 0 or 1\n");
		return -1;
	}

	if (val && !core_if->hwcfg3.b.otg_lpm_en) {
		if(dwc_otg_param_initialized(core_if->core_params->lpm_enable)) {
			cprintf("error: %d invalid for parameter lpm_enable. Check HW configuration.\n",
		     val);
		}
		val = 0;
		retval = -1;
...
int dwc_otg_set_param_tx_thr_length(dwc_otg_core_if_t * core_if, int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 8, 128)) {
		...
		cprintf("warning: tx_thr_length must be 8 - 128\n");
		return -1;
	}
...
int dwc_otg_set_param_rx_thr_length(dwc_otg_core_if_t * core_if, int32 val)
{
	if (DWC_OTG_PARAM_TEST(val, 8, 128)) {
		...
		cprintf("warning: rx_thr_length must be 8 - 128\n");
		return -1;
	}
...
int dwc_otg_set_param_pti_enable(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (val && (core_if->snpsid < OTG_CORE_REV_2_72a)) {
		if (dwc_otg_param_initialized(core_if->core_params->pti_enable)) {
			cprintf("error: %d invalid for parameter pti_enable. Check HW configuration.\n",
			     val);
		}
		retval = -1;
		val = 0;
...
int dwc_otg_set_param_mpi_enable(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if (val && (core_if->hwcfg2.b.multi_proc_int == 0)) {
		if (dwc_otg_param_initialized(core_if->core_params->mpi_enable)) {
			cprintf("error: %d invalid for parameter mpi_enable. Check HW configuration.\n",
			     val);
		}
		retval = -1;
		val = 0;
					...
					int32 val)
{
	...
	if (DWC_OTG_PARAM_TEST(val, 0, 1)) {
		...
		cprintf("warning: ic_usb_cap must be 0 or 1\n");
		return -1;
	}

	if (val && (core_if->hwcfg3.b.otg_enable_ic_usb == 0)) {
		if (dwc_otg_param_initialized(core_if->core_params->ic_usb_cap)) {
			cprintf("error: %d invalid for parameter ic_usb_cap. Check HW configuration.\n",
			     val);
		}
		retval = -1;
		val = 0;
...
int dwc_otg_set_param_ahb_thr_ratio(dwc_otg_core_if_t * core_if, int32 val)
{
	...
	if(DWC_OTG_PARAM_TEST(val, 0, 3)) {
		...
		cprintf("warning: ahb_thr_ratio must be 0 - 3\n");
		return -1;
	}
	...
	if(valid == 0) {
		if(dwc_otg_param_initialized(core_if->core_params->ahb_thr_ratio)) {
			cprintf("error: %d invalid for parameter ahb_thr_ratio. Chack HW configuration.\n", val);
		}
		retval = -1;
		val = 0;
	}
...
void dwc_otg_cil_init(dwc_otg_core_if_t *core_if, const uint32 * reg_base_addr)
{
	...
	host_if->hprt0 =
	    (uint32 *) (reg_base + DWC_OTG_HOST_PORT_REGS_OFFSET);
	...
	for (i = 0; i < MAX_EPS_CHANNELS; i++) {
		core_if->data_fifo[i] =
		    (uint32 *) (reg_base + DWC_OTG_DATA_FIFO_OFFSET +
...
void dwc_otg_flush_tx_fifo(dwc_otg_core_if_t * core_if, const int num)
{
	...
	do {
		...
		if (++count > 10000) {
			cprintf("%s() HANG! GRSTCTL=%0x GNPTXSTS=0x%08x\n",
				 __func__, greset.d32,
